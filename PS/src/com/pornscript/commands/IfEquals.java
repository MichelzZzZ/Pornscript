...
/**
 * 
 *  If two strings/numbers are equal do something
 * 
 */

public class IfEquals implements Command
{
	...
	public void run(String[] args, VariableStore vsRefrence, Scanner scRefrence) throws UnavailableAddressException, TooManyArgumentsException, TooFewArgumentsException 
	{
		...
		if(args.length < 3)
			throw new TooFewArgumentsException("the 'if i have' command expects exactly three arguments");
		
		if(args.length > 3)
			throw new TooManyArgumentsException("the 'if i have' command expects exactly three arguments");
